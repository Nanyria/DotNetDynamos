...
namespace DotNetDynamos
{
    internal partial class Customer : AllUsers
    {
        ...
        //}
        static void NewAccount(AllUsers loggedInCustomer)
        {
            Console.WriteLine("Open new Account");

            // Hämta användarens konton
            if (Account.userAccounts.TryGetValue((Customer)loggedInCustomer, out List<Account> accounts))
            {
                Console.WriteLine("How much money do you want to deposit?");
                decimal balance = Convert.ToDecimal(Console.ReadLine());

                // Skapa ett nytt konto med hjälp av konstruktorn
                Account newAccount = new Account(
                    Account.GenerateAccountNumber(loggedInCustomer._IDnumber),
                    "Default Account Name", // Ange ett lämpligt standardnamn
                    "Default Currency",      // Ange ett lämpligt standardvaluta
                    balance
                );

                // Lägg till det nya kontot i listan av användarens konton
                accounts.Add(newAccount);

                Console.WriteLine("Congrats! Your account has been created successfully!");
                Console.WriteLine($"Here is your new account:" +
                    $"\nAccount Number: {newAccount._accountNumber}" +
                    $"\nBalance: {newAccount._balance}");
            }
            else
            {
                Console.WriteLine("User not found or has no accounts.");
            }
        }
